...
def PLT_PR11_SID(ax, color, wstar, ls, label, buffer=False):
    ...
    for dim in allbin:
        mods =[RUN_SID(df.density[i], \
               ...
               df.ustar[i], \
               df.Lo[i], \
               luc(wstar, df.LAI[i], df.ustar[i], df.Uh[i], df.z0[i], df.z[i], df.d[i], 'PR11').grass(), 'PR11').PR11()[0] for i in df.index]
        lower = min(mods)
        upper = max(mods)
...
def PLT_VGLAI_SID(ax, color, wstar, ls, label, buffer=False):
    ...
    for dim in allbin:
        mods =[RUN_SID(df.density[i], \
               ...
               df.ustar[i], \
               df.Lo[i], \
               luc(wstar, df.LAI[i], df.ustar[i], df.Uh[i], df.z0[i], df.z[i], df.d[i], 'VGLAI').grass(), 'VGLAI').VGLAI()[0] for i in df.index]
        lower = min(mods)
        upper = max(mods)
        median = np.median(mods)
        lowers.append(lower)
...
def PLT_Z01_SID(ax, color, label, buffer=False):
    ...
    for dim in allbin:
        mods =[RUN_SID(df.density[i], \
               ...
               df.ustar[i], \
               df.Lo[i], \
               luc(df.wstar[i], df.LAI[i], df.ustar[i], df.Uh[i], df.z0[i], df.z[i], df.d[i], 'Z01').grass(), 'Z01').Z01()[0] for i in df.index]
        lower = min(mods)
        upper = max(mods)
        median = np.median(mods)
        lowers.append(lower)
...
def PLT_CSU_SID(ax, color, label, buffer=False):
    ...
    for dim in allbin:
        mods =[RUN_SID(df.density[i], \
               ...
               df.ustar[i], \
               df.Lo[i], \
               luc(df.wstar[i], df.LAI[i], df.ustar[i], df.Uh[i], df.z0[i], df.z[i], df.d[i], 'CSU').grass(), 'CSU').CSU()[0] for i in df.index]
        lower = min(mods)
        upper = max(mods)
        median = np.median(mods)
        lowers.append(lower)
...
ax.set_xlabel('Dp(%sm) on grassland' % r'$\mu$', fontsize=7)
...
fig.savefig('../figs/eval_overdp_sid_grass.png', dpi=300)
