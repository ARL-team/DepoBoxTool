...
def PLT_OBS(ax):
    ...
    for k, v in obs.items():
        ...
        dim = v.dim * 1e-6
        vd = v.Vd_cm * 1e-2 #convert cm to m
...
def PLT_PR11_SID(ax, color, wstar, ls, label, buffer=False):
    ...
    for dim in allbin:
        mods =[RUN_SID(df.density[i], \
               ...
               df.ustar[i], \
               df.Lo[i], \
               luc(wstar, df.LAI[i], df.ustar[i], df.Uh[i], df.z0[i], df.z[i], df.d[i], 'PR11').dforest(), 'PR11').PR11()[0] for i in df.index]
        lower = min(mods)
        upper = max(mods)
...
def PLT_VGLAI_SID(ax, color, wstar, ls, label, buffer=False):
    ...
    for dim in allbin:
        mods =[RUN_SID(df.density[i], \
               ...
               df.ustar[i], \
               df.Lo[i], \
               luc(wstar, df.LAI[i], df.ustar[i], df.Uh[i], df.z0[i], df.z[i], df.d[i], 'VGLAI').dforest(), 'VGLAI').VGLAI()[0] for i in df.index]
        lower = min(mods)
        upper = max(mods)
        median = np.median(mods)
        lowers.append(lower)
...
def PLT_Z01_SID(ax, color, label, buffer=False):
    ...
    for dim in allbin:
        mods =[RUN_SID(df.density[i], \
               ...
               df.ustar[i], \
               df.Lo[i], \
               luc(df.wstar[i], df.LAI[i], df.ustar[i], df.Uh[i], df.z0[i], df.z[i], df.d[i], 'Z01').dforest(), 'Z01').Z01()[0] for i in df.index]
        lower = min(mods)
        upper = max(mods)
        median = np.median(mods)
        lowers.append(lower)
...
def PLT_CSU_SID(ax, color, label, buffer=False):
    ...
    for dim in allbin:
        mods =[RUN_SID(df.density[i], \
               ...
               df.ustar[i], \
               df.Lo[i], \
               luc(df.wstar[i], df.LAI[i], df.ustar[i], df.Uh[i], df.z0[i], df.z[i], df.d[i], 'CSU').dforest(), 'CSU').CSU()[0] for i in df.index]
        lower = min(mods)
        upper = max(mods)
        median = np.median(mods)
        lowers.append(lower)
...
#ax.set_yticklabels([0.001, 0.01, 0.1, 1, 10, 100], fontsize=4)
ax.set_ylabel('Vd(cm/s)', fontsize=7)
ax.set_xlabel('Dp(%sm) on deciduous forest' % r'$\mu$', fontsize=7)
...
fig.savefig('../figs/eval_overdp_sid_df.png', dpi=300)
